...
public class LegacyConversionServiceImpl implements LegacyConversionService {
    ...
    private final Map<ClassPair, List<LegacyConversionService>> variants = new HashMap<>();
    private final Map<Class<?>, List<LegacyConversionService>> implClassesToVariants = new HashMap<>();

    private final boolean inner;

    private LegacyConversionServiceImpl(boolean inner) {
        this.inner = inner;
    ...
    public LegacyConversionServiceImpl() {
        this(false);
    }
    ...
    public <A, T extends A, F> void registerCustomPlatformType(Class<A> apiType, MCCConverter<F, T> converter) {
        ...
        LegacyConversionService legacyConversionService = new LegacyConversionServiceImpl(true);
        legacyConversionService.registerPlatformType(apiType, converter);

        ClassPair classPair = new ClassPair(apiType, converter.nativeMinecraftType());
        variants.computeIfAbsent(classPair, classPair1 -> new LinkedList<>()).add(legacyConversionService);
        implClassesToVariants.computeIfAbsent(converter.apiImplementationClass(), aClass -> new LinkedList<>()).add(legacyConversionService);
        knownNativeTypes.add(converter.nativeMinecraftType());
        registerMappings(apiType, converter);
        //registerMappings(apiType, converter);
    }
    ...
    public <F, T> T wrap(F objectToWrap, TypeToken<T> apiTypeToConvertTo) {
        ...
        if (variants.containsKey(classPair)) {
            for (LegacyConversionService legacyConversionService : variants.get(classPair)) {
                T result = legacyConversionService.wrap(objectToWrap, apiTypeToConvertTo);
                if (result != null)
                    return result;
            }
        }
    ...
    public <F, T> F unwrap(T objectToUnwrap, TypeToken<F> nativeTypeToConvertTo) {
        ...
        if (variants.containsKey(classPair)) {
            for (LegacyConversionService legacyConversionService : variants.get(classPair)) {
                F result = legacyConversionService.unwrap(objectToUnwrap, nativeTypeToConvertTo);
                if (result != null)
                    return result;
            }
        }
